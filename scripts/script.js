...
class LojaCosmeticos {
  ...
  // 🎯 SISTEMA SIMPLIFICADO DE WHATSAPP COM CADASTRO
  finalizarCompra() {
    if (this.carrinho.length === 0) {
    ...
    // Mostrar modal de cadastro primeiro
  ...
  configurarModalCadastro() {
    ...
    // Confirmar cadastro e abrir WhatsApp
    if (btnConfirmar) {
    ...
    document.getElementById("cliente-endereco").addEventListener("keypress", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        btnConfirmar.click();
      }
  ...
  // NOVA FUNÇÃO - Gerar mensagem com dados preenchidos
  gerarMensagemWhatsAppComDados(nome, endereco) {
    const itens = this.carrinho.map(item => {
        const precoFinal = item.preco - (item.preco * (item.desconto / 100));
        return `• ${item.quantidade}x ${item.nome} - R$ ${precoFinal.toFixed(2)} cada`;
    }).join('\n');
...
Pedidos só podem ser cancelados por mensagem no mesmo número em até 12 horas após a confirmação. Após esse período, não serão aceitos cancelamentos.`;
  }

  abrirWhatsApp(mensagem) {
    const numeroWhatsApp = "5548999638110"; // Número da sua mulher
